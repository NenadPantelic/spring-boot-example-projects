...
import com.example.caching.redis.app.mapper.UserMapper;
import com.example.caching.redis.app.model.User;
import com.example.caching.redis.app.service.UserService;
...
public class UserServiceImpl implements UserService {
    ...
    @Cacheable(cacheNames = "user:name", condition = "#name!='John'")
    @Override
    ...
    @Cacheable(cacheNames = "user:city", key = "#city", unless = "#result==null or #result.size()==0")
    ...
    // unless=zipCode<=0
    @Cacheable(cacheNames = "user:zipCode", condition = "#zipCode > 0", unless = "#result==null or #result.size()==0")
    @Override
    ...
    @Caching(evict = {
            ...
            @CacheEvict(value = "user:city", key = "#result.getAddress().getCity()", condition = "#result!=null"),
            @CacheEvict(value = "user:name", allEntries = true),
            @CacheEvict(value = "user:zipCode", key = "#result.getAddress().getZipCode()", condition = "#result!=null"),
    })
    @Override
